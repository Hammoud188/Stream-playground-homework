...
public class Homework1 {
    ...
    public IntSummaryStatistics streamPipeline6() {
        return countries.stream()
                .mapToInt(a -> a.getTranslations().values().toArray().length)
                .summaryStatistics();
    }
    ...
    public void streamPipeline7() {
        countries.stream()
                .sorted(Comparator.comparingInt(a -> a.getTimezones().toArray().length))
                .map(Country::getName)
                .forEach(System.out::println);
    }
    ...
    public void streamPipeline8() {
        ...
        countries.stream().
                sorted(Comparator.comparingInt(a -> a.getTimezones().toArray().length))
                .forEach(a -> System.out.println("@"+ a.getCode() + " " + a.getName() + ": " + a.getPopulation()));
    }
...
ed by the language code "es").
    ...
    public long streamPipeline9() {
        return countries.stream()
                .filter(a -> a.getTranslations().containsKey("es") == false)
                .count();
    }
    ...
    public void streamPipeline11() {
        countries.stream()
                .flatMap(a -> a.getTranslations().keySet().stream())
                .distinct()
                .sorted()
                .forEach(System.out::println);
    }
    ...
    public double streamPipeline12() {
        return countries.stream()
                .mapToInt(a -> a.getName().length()).average()
                .getAsDouble();
    }
    ...
    public Optional<Country> streamPipeline14() {
        return countries.stream()
                .filter(a -> a.getArea() != null)
                .max(Comparator.comparing(a -> a.getArea()));
    }
    ...
    public void streamPipeline15() {
        countries.stream().
                filter(a -> a.getArea() != null && a.getArea().compareTo(BigDecimal.TEN) < 0)
                .map(Country::getName)
                .forEach(System.out::println);
    }
    ...
    public void streamPipeline16() {
        countries.stream()
                ...
                .map(Country::getTimezones).distinct()
                .forEach(System.out::println);
    }
